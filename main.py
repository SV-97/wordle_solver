...
def word_of_maximal_entropy(words, known):
        ...
        masked_letter_index = np.vectorize(ord)(masked_words) - ord("a")
        word_entropy = letter_information[masked_letter_index].sum(axis=1) * letter_probability[masked_letter_index].sum(axis=1) / mask.sum()
